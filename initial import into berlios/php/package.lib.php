...
class Package extends Object {
   ...
   function getData($name=FALSE){
   	...
   	$sql="SELECT TO_CHAR(created, 'DD-MON-YYYY') CREATED, ".
	    ...
	    "FROM all_objects ".
	    "WHERE owner = '".$this->owner."' AND object_name='".$name."' AND object_type='PACKAGE' ";
	...
	$sql=sprintf("SELECT TEXT FROM ALL_SOURCE WHERE OWNER='%s' AND TYPE='PACKAGE' AND NAME='%s' ORDER BY line",
		     $this->owner,
		     $name);
	$this->parse($sql);
	$this->execute();
	...
	$sql = "CREATE OR REPLACE package ".$this->owner.".".$sql."/";
	$this->sql=$sql;  
   }
	...
	$sql="SELECT object_name FROM all_objects WHERE owner='%s' AND object_type='PACKAGE' ".
	...
	$sql="SELECT object_name, TO_CHAR(created, 'DD-MON-YYYY') CREATED, ".
	    "TO_CHAR(last_ddl_time, 'DD-MON-YYYY') LASTMODIFIED,  NLS_INITCAP(status) STATUS ".
	    "FROM all_objects WHERE object_type ='PACKAGE' AND ".
