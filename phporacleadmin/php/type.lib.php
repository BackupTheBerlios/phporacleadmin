...
class Type extends Object {
   ...
   function getData($name=FALSE){
   	...
   	$sql="SELECT TO_CHAR(created, 'DD-MON-YYYY') CREATED, ".
	    ...
	    "FROM all_objects ".
	    "WHERE owner = '".$this->owner."' AND object_name='".$name."' AND object_type='TYPE' ";
	...
	$sql=sprintf("SELECT TEXT FROM ALL_SOURCE WHERE OWNER='%s' AND TYPE='TYPE' AND NAME='%s' ORDER BY line",
		     $this->owner,
		     $name);
	...
	if(strlen($sql)!=0){
	    ...
	    $sql = "CREATE OR REPLACE type ".$this->owner.".".$sql."/";
	} else {
	    $sql= "Not availiable.";
	}
	$this->sql=$sql;  
	...
	$sql="SELECT object_name FROM all_objects WHERE owner='%s' AND object_type='TYPE' ".
	...
	$sql="SELECT object_name, TO_CHAR(created, 'DD-MON-YYYY') CREATED, ".
	    "TO_CHAR(last_ddl_time, 'DD-MON-YYYY') LASTMODIFIED,  NLS_INITCAP(status) STATUS ".
	    "FROM all_objects WHERE object_type ='TYPE' AND ".
